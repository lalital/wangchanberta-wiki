...
class DataTrainingArguments:
    ...
    datasets_type: str = field(
        default='LineByLineTextDataset', metadata={'help': 'Type of datasets object.'}
    )
...
def main():
    ...
    if custom_args.ext == 'txt':
        ...
        if data_args.datasets_type == 'LineByLineTextDataset':
            datasets = {
                'train': LineByLineTextDataset(tokenizer, train_files[0], data_args.max_seq_length),
                'validation': LineByLineTextDataset(tokenizer, validation_files[0], data_args.max_seq_length)
            }
    ...
    config = AutoConfig.from_pretrained(
        pretrained_model_name_or_path=arch_args.architecture,
